...
{
  // ALLOCATION WILL BE DONE BY MEASURE

  // float totalGB = ((nLayers - 1) * mGpuParams.maxTrackletsPerCluster * mGpuParams.clustersPerROfCapacity * 2304 * sizeof(Tracklet) +
  //                  (nLayers - 2) * mGpuParams.maxNeighboursSize * 2304 * sizeof(Cell) +
  //                  sizeof(Road<nLayers - 2>) * mGpuParams.maxRoadPerRofSize * 2304 +
  //                  (nLayers - 1) * sizeof(Tracklet*) +
  //                  (nLayers - 2) * sizeof(Cell*)) /
  //                 GB;
  // LOGP(info, "Hybrid SA fitting allocating: {:.3} GB so far.", totalGB);
  // // Tracklets
  // for (auto iLayer{0}; iLayer < nLayers - 1; ++iLayer) {
  //   //   allocMemAsync(reinterpret_cast<void**>(&mTrackletsDevice[iLayer]), mGpuParams.maxTrackletsPerCluster * mGpuParams.clustersPerROfCapacity * 2304 * sizeof(Tracklet), nullptr);
  //   allocMemAsync(reinterpret_cast<void**>(&mTrackletsDeviceArray), (nLayers - 1) * sizeof(Tracklet*), nullptr);
  // }
  // // Cells
  // for (auto iLayer{0}; iLayer < nLayers - 2; ++iLayer) {
  //   allocMemAsync(reinterpret_cast<void**>(&mCellsDevice[iLayer]), mGpuParams.maxNeighboursSize * 2304 * sizeof(Cell), nullptr);
  //   allocMemAsync(reinterpret_cast<void**>(&mCellsDeviceArray), (nLayers - 2) * sizeof(Cell*), nullptr);
  // }
  // // Roads
...
void TimeFrameGPU<nLayers>::loadUnsortedClustersDevice()
{
  for (auto iLayer{0}; iLayer < nLayers; ++iLayer) {
    ...
    allocMemAsync(reinterpret_cast<void**>(&mUnsortedClustersDevice[iLayer]), mUnsortedClusters[iLayer].size() * sizeof(Cluster), nullptr);
  ...
  allocMemAsync(reinterpret_cast<void**>(&mUnsortedClustersDeviceArray), nLayers * sizeof(Cluster*), nullptr);
}
...
void TimeFrameGPU<nLayers>::loadClustersDevice()
{
  for (auto iLayer{0}; iLayer < nLayers; ++iLayer) {
    ...
    allocMemAsync(reinterpret_cast<void**>(&mClustersDevice[iLayer]), mClusters[iLayer].size() * sizeof(Cluster), nullptr);
    // Register and move data
    checkGPUError(cudaHostRegister(mClusters[iLayer].data(), mClusters[iLayer].size() * sizeof(Cluster), cudaHostRegisterPortable));
  ...
  allocMemAsync(reinterpret_cast<void**>(&mClustersDeviceArray), nLayers * sizeof(Cluster*), nullptr);
}
...
void TimeFrameGPU<nLayers>::loadTrackingFrameInfoDevice()
{
  for (auto iLayer{0}; iLayer < nLayers; ++iLayer) {
    ...
    allocMemAsync(reinterpret_cast<void**>(&mTrackingFrameInfoDevice[iLayer]), mTrackingFrameInfo[iLayer].size() * sizeof(TrackingFrameInfo), nullptr);
    // Register and move data
  ...
  allocMemAsync(reinterpret_cast<void**>(&mTrackingFrameInfoDeviceArray), nLayers * sizeof(TrackingFrameInfo*), nullptr);
}
...
void TimeFrameGPU<nLayers>::loadTrackletsDevice()
{
  for (auto iLayer{0}; iLayer < nLayers - 1; ++iLayer) {
    ...
    allocMemAsync(reinterpret_cast<void**>(&mTrackletsDevice[iLayer]), mTracklets[iLayer].size() * sizeof(Tracklet), nullptr);
    // Register and move data
...
void TimeFrameGPU<nLayers>::loadCellsDevice()
{
  for (auto iLayer{0}; iLayer < nLayers - 2; ++iLayer) {
    ...
    allocMemAsync(reinterpret_cast<void**>(&mCellsDevice[iLayer]), mCells[iLayer].size() * sizeof(Cell), nullptr);
    // Register and move data
...
void TimeFrameGPU<nLayers>::loadTrackSeedsDevice()
{
  for (auto iLayer{0}; iLayer < nLayers - 2; ++iLayer) {
    ...
    allocMemAsync(reinterpret_cast<void**>(&mCellSeedsDevice[iLayer]), mCellSeeds[iLayer].size() * sizeof(o2::track::TrackParCovF), nullptr);
    // Register and move data
  ...
  allocMemAsync(reinterpret_cast<void**>(&mCellSeedsDeviceArray), (nLayers - 2) * sizeof(o2::track::TrackParCovF*), nullptr);
...
void TimeFrameGPU<nLayers>::loadTrackSeedsChi2Device()
{
  for (auto iLayer{0}; iLayer < nLayers - 2; ++iLayer) {
    ...
    allocMemAsync(reinterpret_cast<void**>(&mCellSeedsChi2Device[iLayer]), mCellSeedsChi2[iLayer].size() * sizeof(float), nullptr);
    // Register and move data
  ...
  allocMemAsync(reinterpret_cast<void**>(&mCellSeedsChi2DeviceArray), (nLayers - 2) * sizeof(float*), nullptr);
...
void TimeFrameGPU<nLayers>::loadRoadsDevice()
{
  ...
  allocMemAsync(reinterpret_cast<void**>(&mRoadsDevice), mRoads.size() * sizeof(Road<nLayers - 2>), nullptr);
