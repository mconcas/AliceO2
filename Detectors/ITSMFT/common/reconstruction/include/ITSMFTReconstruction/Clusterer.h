 ...
 public:
  ...
  struct BBox {
    ...
    bool hasBBoxOverlap(uint16_t row, uint16_t col, uint16_t h, uint16_t w) const
    {
      ...
      auto dJ = rowMin - row;
      if (dI <= 0) {
        iResult = std::abs(dI) <= colMax - colMin ? true : false; // width of comparing cluster
      } else {
        iResult = dI <= w ? true : false; // width of compared cluster
      }
      // Check y
      if (dJ <= 0) {
        jResult = std::abs(dJ) <= rowMax - rowMin ? true : false;
      } else {
        jResult = dJ <= h ? true : false;
      }
  ...
  struct ClusterInfo {
    ClusterInfo() = default;
    ClusterInfo(o2::itsmft::ClusterPattern& p, uint16_t i, uint16_t j) : col{i}, row{j}
    {
      pattern = p; // not possible to use cctor
    }
  ...
  struct ClustererThread {
    ...
    void finishChipSingleHitFast(uint32_t hit, ChipPixelData* curChipData, CompClusCont* compClusPtr,
                                 PatternCont* patternsPtr, const ConstMCTruth* labelsDigPtr, MCTruth* labelsClusPTr, gsl::span<const o2::itsmft::ROFRecord>& prevROFs);
    void process(uint16_t chip, uint16_t nChips, CompClusCont* compClusPtr, PatternCont* patternsPtr,
                 const ConstMCTruth* labelsDigPtr, MCTruth* labelsClPtr, const ROFRecord& rofPtr, gsl::span<const o2::itsmft::ROFRecord>& prevROFs);
